//6-5_MILESTONE-LIGHTING_COMPLEX_OBJECTS-DLUDWIG_02-18-2024
//modified https://github.com/lorenarms/SNHU_CS_330_Computer_Graphics_and_Visualization/tree/master/7-1%20Final%20Project%20-%20A%203D%20Scene
...
//window title
const char* const WINDOW_TITLE = "CS330-3D-SCENE-2024_DLUDWIG";
...
glm::vec3 gSpotLightColor(0.6f, 1.0f, 1.0f);
glm::vec3 gSpotLightPosition(2.75f, 4.0f, 1.5f);
glm::vec3 gSpotLightScale(0.5f);

// Light color, position and scale
glm::vec3 gKeyLightColor(0.6f, 0.0f, 1.0f);
glm::vec3 gKeyLightPosition(2.0f, 4.0f, 1.0f);
...
void UDestroyTexture(GLuint textureId1);   /*------------------------*/
...
bool UCreateTexture(const char* filename, GLuint& textureId1); /*------------------------*/
void UCreateLightMesh(GLightMesh& lightMesh); 
...
void main()
{
	...
	float spotStrength = 0.5f; // Set ambient or global lighting strength
...
void UProcessInput(GLFWwindow* window)
{
	...
	// Add color to key light [ ]
	if (glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS)
	{
		gKeyLightColor.r = 2.5f;
		gKeyLightColor.g = 0.0f;
		gKeyLightColor.b = 2.5f;

	}

	// Turn off key light color [ ]
	if (glfwGetKey(window, GLFW_KEY_L) == GLFW_PRESS)
	{
		gKeyLightColor.r = 0.6f;
		gKeyLightColor.g = 0.0f;
...
void URender(vector<GLMesh> scene)
{
	...
	constexpr float angularVelocity = glm::radians(60.0f);
	if (gSpotLightOrbit)
	{
		glm::vec4 newPosition = glm::rotate(angularVelocity * gDeltaTime, glm::vec3(0.0f, 1.0f, 0.0f)) * glm::vec4(gSpotLightPosition, 0.5f);
